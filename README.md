...
- **`config.py`**: 모든 설정값과 전략 변수를 관리하는 파일.
- **`main.py`**: 모든 모듈을 조립하고 메인 루프를 실행함.
- **`strategy.py`**: 보조지표와 규칙에 따라 '매수/매도/대기'를 결정하는 핵심 전략.
- **`indicators.py`**: 차트 데이터를 받아 매매 전략에 필요한 모든 기술적 보조지표를 계산함.
- **`trader.py`**: '매매 결정'을 받아 실제 거래소에 주문(진입, TP, SL)을 실행함.
- **`utils.py`**: 슬랙 알림, CSV 기록, 포지션 조회 등 여러 곳에서 쓰이는 유틸.
