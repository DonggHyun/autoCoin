...
def check_entry_signal(dfs, last_signal, last_timestamp, last_market_condition, symbol=''):
    ...
    if market_condition == 'trending':
        ...
        if now_1m['rsi'] > 55: long_score += weights.get('rsi_momentum_sustain', 0)
        if now_1m['rsi'] < 45: short_score += weights.get('rsi_momentum_sustain', 0)
        ...
        if 'bollinger_lower' in now_1m and now_1m['close'] > now_1m['bollinger_lower'] and prev_1m['close'] <= prev_1m['bollinger_lower']: long_score += weights.get('bollinger_reversal', 0)
        if 'bollinger_upper' in now_1m and now_1m['close'] < now_1m['bollinger_upper'] and prev_1m['close'] >= prev_1m['bollinger_upper']: short_score += weights.get('bollinger_reversal', 0)
    ...
    if 'r1' in now_1m and now_1m['close'] > now_1m['r1'] and prev_1m['close'] <= prev_1m['r1']: long_score += weights.get('pivot_breakout', 0)
    if 's1' in now_1m and now_1m['close'] < now_1m['s1'] and prev_1m['close'] >= prev_1m['s1']: short_score += weights.get('pivot_breakout', 0)
    ...
    if 'prev_day_high' in now_1m and now_1m['close'] > now_1m['prev_day_high'] and prev_1m['close'] <= prev_1m['prev_day_high']: long_score += weights.get('pdhl_breakout', 0)
    if 'prev_day_low' in now_1m and now_1m['close'] < now_1m['prev_day_low'] and prev_1m['close'] >= prev_1m['prev_day_low']: short_score += weights.get('pdhl_breakout', 0)
